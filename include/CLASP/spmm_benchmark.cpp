...
#include <cublas_v2.h>
#include "projects/sputnik/sputnik/sputnik.h"
#include <cusparse.h>
...
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, bool sorted, bool func, int sparse){
    ...
    if (sparse == 1){
        ...
        MakeDenseMatrix<InType>(n, k, rhs_matrix, generator); //TODO: init dense matrix with static values
        ...
        MakeDenseMatrix<InType>(1, nonzeros, values, generator); //TODO: init sparse matrix with static values 
        /* for(auto i=0; i<64; i++){
            float temp = distribution(generator);
        ...
        if (kernel == 0){
            printf("Using WMMA \n");
            spmm_amp::wmmaSpmm(m_vec, vec_length, k, n, d_row_indices, d_row_offsets, d_col_indices, d_value, d_rhs_matrix, d_output_value);
        }
        else if (kernel == 1){
            printf("Using CUDA \n");
            spmm_amp::cudaSpmm(m_vec, vec_length, k, n, d_row_indices, d_row_offsets, d_col_indices, d_value, d_rhs_matrix, d_output_value);
        }
        else if (kernel == 2){
        ...
        if (func){
            ...
            for (int j=0; j < m * k; j++){
                ...
                if (abs((float)output_value_cuda[j] - (float)output_value_host[j]) > 1){//0.5){
                    // if (j < 2560) 
