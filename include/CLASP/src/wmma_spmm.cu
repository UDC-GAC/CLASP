...
#include "spmm_utils/output_tile_base.h"
#include <stdio.h>
#include <mma.h>
    ...
    OutType* __restrict__ output_matrix)
{
    ...
    __shared__ float4 values_tile_array[VecLength * Tile_N/4];
    ...
    constexpr int kDenseFragmentSize = Tile_N / 8 * (16);
    ...
    constexpr int InnerSteps = Tile_N / 8;
    ...
    OutType* __restrict__ output_matrix)
{
    ...
    // Register fragment for the dense matrix values
    constexpr int kDenseFragmentSize = Tile_N / 8 * (16); //(8*2) en cada iteracion se carga el doble de elementos, pero en computo total es el mismo tamaño
    ...
    wmmaOutputTile4<OutType, StoreType> output_tile_storer(lane_id, thread_group, m_index_vec, k_index, k, output_fragment, output_matrix);
    output_tile_storer.Store();
}

template <typename LoadType, typename IndexType, typename VecType, typename OutType, typename StoreType, int Tile_N, int Tile_K, int BlockWidth, int VecLength=2>
    ...
    OutType* __restrict__ output_matrix)
{
    ...
    wmmaOutputTile2<OutType, StoreType> output_tile_storer(lane_id, thread_group, m_index_vec, k_index, k, output_fragment, output_matrix);
    output_tile_storer.Store();
}
    ...
    float* __restrict__ output_matrix)
{
    ...
    switch(vec_length){
        ...
        case 4:
            printf("V=4\n");
            wmmaSpmmKernel4<float4, int, float2, float, float4, Tile_N, Tile_K, BlockWidth, 4><<<grid_dim, block_dim>>>(
                m_vec, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
            break;
        case 8:
            printf("V=8\n");
    ...
    half* __restrict__ output_matrix)
{
    ...
    switch(vec_length){
        case 2:
            //printf("V=2\n");
            wmmaSpmmKernel2<float4, int, float, half, float4, Tile_N, Tile_K, BlockWidth, 2><<<grid_dim, block_dim>>>(
                m_vec, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
            break;
        case 4:
            //printf("V=4\n");
            wmmaSpmmKernel4<float4, int, float2, half, float4, Tile_N, Tile_K, BlockWidth, 4><<<grid_dim, block_dim>>>(
                m_vec, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
            break;
        case 8:
            //printf("V=8\n");
            wmmaSpmmKernel8<float4, int, float4, half, float4, Tile_N, Tile_K, BlockWidth, 8><<<grid_dim, block_dim>>>(
                m_vec, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
            break;
        default:
            printf("Unsupported Vector Length!\n");
